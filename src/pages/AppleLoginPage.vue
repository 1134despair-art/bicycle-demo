...
import { Apple, ChevronLeft } from 'lucide-vue-next'
...
const completeLogin = () => {
  store.showToast(localeStore.t('third.apple.successToast'))
...
<template>
  <AuthPageFrame content-class="pb-12">
    ...
    <ThirdPartyBrandPanel
      accent-class="bg-#111111"
      :title="localeStore.t('third.apple.title')"
      :subtitle="localeStore.t('third.apple.subtitle')"
      :notice="localeStore.t('third.apple.notice')"
    >
      <template #badge>
        <Apple :size="24" :stroke-width="2.2" />
      </template>
      ...
      <template #action>
        <button
          ...
          @click="completeLogin"
        >
          {{ localeStore.t('third.apple.action') }}
        </button>
      </template>
    </ThirdPartyBrandPanel>
